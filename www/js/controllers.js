...
CLIENT
STOCK
SUPERVISION DE DESTOCKEUR
RECUPERATION DE MARCHANDISES
DECHARGEMENT
GEOLOCALISATION DESTOCKEURS
CONSULTATION SOLDE
PLANNING DESTOCKEURS*/
    $scope.permission = function (module) {
      return checkPermissionModule.getPermissionModule(module);
    }
    ...
    $scope.goToTauxDePresences = function () {
      $state.transitionTo(
        "app.tauxdepresences",
        {},
        {
  ...
  .controller("CoordonneesCtrl", function (
    ...
    $scope.getPosition = function () {
      ...
      $scope.positionClient = position.position;
      $scope.adresse = position.adresse;
  ...
  .controller("MapCtrl", function (
                ...
                google.maps.event.addListenerOnce($scope.map, 'idle', function () {
                  ...
                  var infoWindow1 = new google.maps.InfoWindow({
                    content: "Ma position actuelle!" +
                      "<br/>Nom: " + $scope.data.agentchoisit.prenom + $scope.data.agentchoisit.nom
                      + "<br/>Date: " + date
                  });
    ...
    $scope.getOptLocalite = function (option) {
      //   console.log($scope.data.regionchoisit)
      return option;
    };
    ...
    $scope.initMap = function () {
      ...
      if (user && user.code) {
        var codeChef = { codeChefzone: user.code };
        $scope.data.NomChef = user.nom;
        $scope.data.PrenomChef = user.prenom;
        console.log('---------Dernier Position Agent ----------');
        console.log(codeChef);
        ApiDernierPositionAgent.ListDernierPositionAgent(codeChef)
          .success(function (response) {
            ...
            $cordovaGeolocation.getCurrentPosition(options).then(function (position) {
              ...
              google.maps.event.addListenerOnce($scope.map, "idle", function () {
                ...
                var infoWindow = new google.maps.InfoWindow({
                  content: "Ma position actuelle!" +
                    "<br/>Nom: " + $scope.data.PrenomChef + $scope.data.NomChef
                    + "<br/>Date: " + date
                });
    ...
    /***********debut************ */
    $scope.initMapAllAgent = function () {
      var user = JSON.parse(localStorage.getItem("user"));
      /******************/
      if (user && user.code) {
        var codeChef = { codeChefzone: user.code };
          ...
          .success(function (response) {
            ...
            for (var i = 0; i < $scope.data.dernierPositionAgent.length; i++) {
              if ($scope.data.dernierPositionAgent[i].latitude !== " " && $scope.data.dernierPositionAgent[i].longitude !== " ") {
                $scope.data.positionAgents = $scope.data.dernierPositionAgent[i].latitude + "," + $scope.data.dernierPositionAgent[i].longitude;
                coordonate.push({
                  lat: + $scope.data.dernierPositionAgent[i].latitude,
                  long: + $scope.data.dernierPositionAgent[i].longitude
                });

                var latLngAll = new google.maps.LatLng(coordonate.lat, coordonate.long);
                var mapOptionAll = {
                  center: latLngAll,
                  zoom: 12,
                  mapTypeId: google.maps.MapTypeId.ROADMAP,
                ...
                Agent.push({
                  nom: $scope.data.dernierPositionAgent[i].nom,
                  dateEnregistrement: $scope.data.dernierPositionAgent[i].dateEnregistrement
                });
              }
            };
            ...
            console.log("latLngAll", latLngAll);
              ...
              google.maps.event.addListenerOnce($scope.map, "idle", function () {
                ...
                for (var i = 0; i < coordonate.length; i++) {
                  ...
                  var infoWindowAll = new google.maps.InfoWindow({
                    content: "Ma Derniere position! "
                      + "<br/>Nom: " + Agent[i].nom
                      + "<br/>Date: " + Agent[i].dateEnregistrement

                  });

                  google.maps.event.addListener(markerAll, "click", function () {
                    infoWindowAll.open($scope.map, markerAll);
                  });
                }
  ...
  .controller("StockCtrl", function (
    ...
    ApiListStock.getListStock().success(
      ...
      function (response) {
        ...
        if (response) {
          $scope.data.listStock = response;
          console.log("liststock", $scope.data.listStock);
          for (var i = 0; i < $scope.data.listStock.length; i++) {
            if ($scope.data.listStock.quantite != " ") {
              $scope.data.listStock.quantite = parseInt($scope.data.listStock[i].quantite);
            } else {
  ...
  .controller("ClientCtrl", function (
    ...
    $scope.goTogoogle = function (cli) {
      ...
      ApiListClient.getListClient().success(
        function (response) {
          $ionicLoading.hide();
          if (response) {
            $scope.data.positionclients = response;
          }
          console.log(response);
        },
        (error) => {
    ...
    $scope.goToNewClient = function (item = null, sens) {
      ...
      //  var sens = JSON.parse(localStorage.getItem("sens"));
  ...
  .controller("AddClientCtrl", function (
    ...
    $scope.getAdresse = function (latlng) {
      $scope.data.adresseUser;
      $scope.nomclient = user.nom;
      var options = { timeout: 10000, enableHighAccuracy: true };
    ...
    $scope.initvar = function () {
      ...
      $scope.data.listdepartements = [];
      $scope.data.listlocalites = [];
      $scope.data.listTypePointVentes = [];
      $scope.data.regionchoisit = null;
      $scope.data.villechoisit = null;
      ...
      $scope.data.departementchoisit = null;
      $scope.data.localitechoisit = null;
      $scope.data.TypePointVentechoisit = null;
    ...
    $scope.getOptDepartement = function (option) {
      return option;
    };
    $scope.getOptLocalite = function (option) {
      return option;
  ...
  .controller("PdcCtrl", function (
    ...
    $scope.initvar = function () {
      ...
      ApiListPdc.getListPdc(codeClient).success(
        function (response) {
          $ionicLoading.hide();
          if (response) {
            console.log('------PRC LOCAL');
            console.log($scope.data.pdcs)
            $scope.data.pdcs = $scope.data.pdcs.concat(response);
              ...
              .success(
                function (response) {
                  ...
                  if (response.reponse == 1) {
                    ...
                    for (var i = 0; i < $scope.data.pdcs.length; i++) {
                      if (
                        $scope.data.pdcs[i].codePRC ===
                        $scope.itemEdit.codePRC
                      ) {

                        $scope.data.pdcs.splice(i, 1);
                        var prclocal = JSON.parse(localStorage.getItem('prclocal'));
    ...
    $scope.goToDetailPdc = function (pdc) {
      console.log("----------------------Detail-------------------");
      console.log(pdc);
      localStorage.setItem("pdc-detail", JSON.stringify(pdc));
      $state.transitionTo(
        "app.details-pdc",
        {},
        {
    ...
    $scope.goToNewpdc = function () {
      sessionStorage.setItem("newPdc", true);
      $state.transitionTo(
        "app.nouvel-pdc",
        {},
        {
  ...
  .controller("NewPdcCtrl", function (
    $scope,
    $state,
    $ionicLoading,
    ApiListPrc,
    ...
    $scope.editDetail = function (item, action) {
      ...
      for (var i = 0; i < $scope.data.detailsPDC.length; i++) {
        if (
          $scope.data.detailsPDC[i].idMotif === "edit" &&
          $scope.data.detailsPDC[i].codeArticle !== item.codeArticle
        ) {
          $scope.data.detailsPDC[i].idMotif = 0;
        }
      }
      $scope.itemEdit = item;
    };
    ...
    $scope.valideEdit = function () {
      if ($scope.data.motifchoisit && $scope.data.motifchoisit.idMotif !== "") {
        for (var i = 0; i < $scope.data.detailsPDC.length; i++) {
          if (
            $scope.data.detailsPDC[i].idMotif === "edit" &&
            $scope.data.detailsPDC[i].codeArticle ===
            ...
            ApiModificationDetailPDC.modificationDetailPDC(ligneDetailTosend)
              .success(
                function (response) {
                  ...
                  if (response.reponse == 1) {
                    ...
                    console.log(index)
                    $scope.data.detailsPDC[index].idMotif =
                      $scope.data.motifchoisit.idMotif;
                    $scope.data.detailsPDC[index].quantite = $scope.data.quantite;
                    $scope.data.detailsPDC[index].prix = $scope.data.prix;
                    $scope.data.detailsPDC[index].codeArticle = $scope.data.artcilechoisit.code;
                    $scope.data.detailsPDC[index].article = $scope.data.artcilechoisit.libelle;
    ...
    $scope.deletObject = function (item) {
      let index = $scope.data.detailsPDC.indexOf(item);

      if (index > -1) {
        $scope.data.detailsPDC.splice(index, 1);
      }
    };
  ...
  .controller("DetailPdcCtrl", function (
    $scope,
    $state,
    $ionicLoading,
    ApiListPdc,
    ...
    $scope.initvar = function () {
      ...
      $scope.data.pdc = JSON.parse(localStorage.getItem("pdc-detail"));
    ...
    $scope.editDetail = function (item, action) {
      ...
      for (var i = 0; i < $scope.data.detailsPDC.details.length; i++) {
        if (
          $scope.data.detailsPDC.details[i].idMotif === "edit" &&
          $scope.data.detailsPDC.details[i].codeArticle !== item.codeArticle
        ) {
          $scope.data.detailsPDC.details[i].idMotif = 0;
        }
      }
      $scope.itemEdit = item;
    };
    ...
    $scope.valideEdit = function () {
      if ($scope.data.motifchoisit && $scope.data.motifchoisit.idMotif !== "") {
        for (var i = 0; i < $scope.data.detailsPDC.details.length; i++) {
          if (
            $scope.data.detailsPDC.details[i].idMotif === "edit" &&
            $scope.data.detailsPDC.details[i].codeArticle ===
            $scope.itemEdit.codeArticle
          ) {
            var index = i;
            ...
            var ligneDetailTosend = {
              codePDC: $scope.data.detailsPDC.codePDC,
              codeDetail: $scope.itemEdit.codeDetail,
              codeArticle: $scope.data.artcilechoisit.code,
              quantite: +$scope.data.quantite,
              isCanceled: 1,
              ...
              .success(
                function (response) {
                  ...
                  if (response.reponse == 1) {
                    ...
                    console.log(index)
                    $scope.data.detailsPDC.details[index].idMotif =
                      $scope.data.motifchoisit.idMotif;
                    $scope.data.detailsPDC.details[index].quantite = $scope.data.quantite;
                    $scope.data.detailsPDC.details[index].prix = $scope.data.prix;
                    $scope.data.detailsPDC.details[index].codeArticle = $scope.data.artcilechoisit.code;
                    $scope.data.detailsPDC.details[index].article = $scope.data.artcilechoisit.libelle;
      ...
      if ($scope.data.motifchoisit && $scope.data.motifchoisit.idMotif !== "") {
        $ionicPopup.show({
          ...
          if (result) {
            ...
            ApiDeletDetailPDC.deletDetailPDC(ligneDetailTosend)
              .success(
                function (response) {
                  ...
                  if (response.reponse == 1) {
                    ...
                    for (var i = 0; i < $scope.data.detailsPDC.details.length; i++) {
                      if (
                        ...
                        $scope.data.detailsPDC.details.splice(i, 1);

                        var pdclocal = JSON.parse(localStorage.getItem('pdclocal'));

                        if (pdclocal && pdclocal.length > 0) {
                          var pdcToModif = $filter('filter')(pdclocal, { codePDC: $scope.data.detailsPDC.codePDC });

                          if (pdcToModif && pdcToModif.length == 1) {
                            pdclocal.splice(pdcToDelet[0], 1)

                            $scope.data.detailsPDC.detailsPDC = $scope.data.detailsPDC.details;
                            $scope.data.detailsPDC.details = $scope.data.detailsPDC.details;

                            pdclocal.push($scope.data.detailsPDC)

                            localStorage.setItem('pdclocal', JSON.stringify(pdclocal));
                          }
    ...
    $scope.getDetailPdc = function () {
      $scope.data.montantDtails = 0;



      if ($scope.data.pdc && !$scope.data.pdc.local) {

        ApiDetailPdc.getDetailPdc($scope.data.pdc.codePDC).success(function (
          response
        ) {
          if (response) {
            console.log("---------------------Detail pdc--------------");
        ...
        //  var prclocal = JSON.parse(localStorage.getItem('prclocal'));
        $scope.data.detailsPDC = $scope.data.pdc;
        $scope.data.detailsPDC.details = $scope.data.pdc.detailsPDC;
    ...
    $scope.ajouter = function () {
      ...
      $scope.data.detailsPDC.push({
        index: $scope.data.detailsPDC.length + 1,
        codeDetail: "DPDC" + $scope.data.user.code + $scope.getCodePrc(),
        codeArticle: $scope.data.artcilechoisit.code,
        quantite: $scope.data.quantite,
        isCanceled: false,
        artcilechoisit: $scope.data.artcilechoisit,
  ...
  .controller("detailsPlanningUserCtrl", function (
    $scope,
    $state,
    $ionicLoading,
    ApiListPrc,
    ...
    console.log('-------- details planning user-------');

  })

  .controller("PlanningUserCtrl", function (
    $scope,
    $state,
    $ionicLoading,
    ApiListPrc,
    ...
    $scope.data.listPlanningUsers = [];
    $scope.data.UtilisateurChoisit;
    $scope.data.datefilterDebut;
    $scope.data.datefilterFin;
    $scope.data.listUsers = [];

    $scope.selectUser = function () {
      ApiListPlanningUser.ListPlanningUser()
        .success(function (reponse) {
          console.log('-----Liste Utilisateu');
          $scope.data.listPlanningUsers = reponse;
          console.log($scope.data.listPlanningUsers)
        })

    }
    $scope.selectUser();
    $scope.getOptUser = function (option) {
      return option;
    };
    $scope.selectPlanningUser = function () {
      console.log('-----Liste Planning Agent');

      if ($scope.data.UtilisateurChoisit && $scope.data.datefilterDebut && $scope.data.datefilterFin) {
        console.log("choix ", $scope.data.UtilisateurChoisit.codeAgent);
        var parametres = {
          codeAgent: $scope.data.UtilisateurChoisit.codeAgent,
          dateDebut: $scope.data.datefilterDebut,
          dateFin: $scope.data.datefilterFin
        }
        ...
        ApiPlanningAgent.ListPlanningAgent(parametres)
          .success(function (reponse) {
            console.log('-----Liste Agent utilisateur');
            $scope.data.listUsers = reponse;
            console.log($scope.data.listUsers)
          })
        $state.transitionTo(
          "app.details-planninguser",
          {},
          {
            reload: true,
            inherit: false,
  ...
  .controller("AddPdsCtrl", function (
    ...
    $scope.ajouter = function () {
      ...
      // if ($scope.data.quantite >= 3000) {

      //   $ionicPopup.show({
      //     title: 'Alert ',
      //     template: 'Desolé! Cette quantité dépasse la limite.',
      //     scope: $scope,
      //     buttons: [
      //       {
      //         text: 'OK',
      ...
      // if ($scope.data.prix <= 1000) {
      //   $ionicPopup.show({
      //     title: 'Alert ',
      //     template: 'Desolé! Cet article ne vaut pas ce prix.',
      //     scope: $scope,
      //     buttons: [
      //       {
      //         text: 'OK',
      ...
      errorInput = $scope.data.quantite >= 3000 ? 'Desolé! Cette quantité dépasse la limite.' : errorInput;
      errorInput = $scope.data.prix <= 1000 ? 'Desolé! Cet article ne vaut pas ce prix.' : errorInput;
      errorInput = !$scope.data.artcilechoisit ? 'Desolé! Veuillez ajouter un article.' : errorInput;
      errorInput = !$scope.data.quantite ? 'Desolé! Veuillez ajouter une quantité.' : errorInput;
      errorInput = !$scope.data.prix ? 'Desolé! Veuillez ajouter un prix.' : errorInput;
  ...
  .controller("FacturationCtrl", function (
    ...
    $scope.submit = function () {
      ...
      errorInput = $scope.data.fact.prix > prix ? 'Le prix de la vente supérieur au prix du destockage  .' : errorInput
  ...
  //  Taux de presence controller
  .controller('TauxdepresencesCtrl', function (
    $scope, $state, $ionicLoading,
    ApiListPrc, ApiDetailPrc, ApiAjoutPrc,
    ApiListClient, ApiListMotif,
    ApiListArticle, $ionicPopup,
    ...
    ApiListGrossisteDestocke, $filter, ApiListventejour, ApiDetailsTauxPresences) {
    ...
    $scope.goToNewTaux = function () {
      localStorage.setItem('detailstauxpresence', null)
      $state.transitionTo(
        "app.tauxpresence",
        {},
        {
    ...
    $scope.goToModifTaux = function (codeTauxpresence) {
      var codeTauxpresence = {
        codeTauxpresence: codeTauxpresence
      }
      $ionicLoading.show({
        template: 'En cours...'
      });
      ApiDetailsTauxPresences.getDetailsTauxPresences(codeTauxpresence)
        .success(function (response) {
          ...
          if (response) {
            if (response.details) {
              localStorage.setItem('detailstauxpresence', JSON.stringify(response))
              $state.transitionTo(
                "app.tauxpresence",
                {},
                {
  ...
  .controller('DetailTauxdepresencesCtrl', function (
    ...
    $scope.editDetail = function (item, action) {
      ...
      for (var i = 0; i < $scope.details.details.length; i++) {
        if (
          $scope.details.details[i].idMotif !== "edit" &&
          $scope.details.details[i].codeArticle !==
          $scope.itemEdit.codeArticle
        ) {
          $scope.details.details[i].idMotif = null;
        }
      }
    ...
    $scope.valideEdit = function () {
      var details = [];
      for (var i = 0; i < $scope.details.details[i].length; i++) {
        var detail = {
          codeArticle: $scope.details.details[i].codeArticle,
        }
        if ($scope.details.details[i].idMotif === "edit" && $scope.details.details[i].codeArticle === $scope.itemEdit.codeArticle
        ) {
          if ($scope.itemEdit.present == true) {
            detail['present'] = 1;
          } else {
            detail['present'] = 0;
          }
          if ($scope.itemEdit.ouvert == true) {
            detail['ouvert'] = 1;
          } else {
            detail['ouvert'] = 0;
          }
          ...
          detail['present'] = $scope.details.details[i].present;
          detail['ouvert'] = $scope.details.details[i].ouvert;
        }
        details.push(detail);
      }
      ...
      var ligneDetailTosend = {
        codeTauxpresence: $scope.details.codeTauxpresence,
        codeClient: $scope.details.codeClient,
        idOperation: $scope.details.idOperation,
        raisonSociale: $scope.details.raisonSociale,
        adresse: $scope.details.adresse,
  ...
  .controller('TauxpresenceCtrl', function (
    ...
    ApiListGrossisteDestocke, $filter, ApiListventejour, ApiDetailsTauxPresences) {
    console.log('contoller Tauxpresence');

    $scope.data.recuperationarticle = {};
    $scope.data.listarticles = [];
    $scope.data.artcilechoisit = null;
    ...
    $scope.editDetail = function (item, action) {
      ...
      for (var i = 0; i < $scope.data.detailsPDC.length; i++) {
        if (
          ...
          $scope.data.detailsPDC[i].idMotif = null;
        }
      }
    ...
    $scope.initvar = function () {
      ...
      $scope.data.telephone = null;
      $scope.data.adresse = null;
      $scope.data.raisonSociale = null;

      var details = localStorage.getItem("detailstauxpresence");
      $scope.details = JSON.parse(details)

      if ($scope.details) {
        console.log('Il est la');
        $scope.codeTauxpresence = $scope.details.codeTauxpresence;
        $scope.data.telephone = $scope.details.telephone;
        $scope.data.adresse = $scope.details.adresse;
        $scope.data.raisonSociale = $scope.details.raisonSociale;
      }
    ...
    ApiListClient.getListClient().success(function (response) {
      ...
      if (response) {
        ...
        if ($scope.details && $scope.details.codeClient) {
          var searchClient = $filter('filter')($scope.data.listtauxclients, { codeClient: $scope.details.codeClient })
          if (searchClient && searchClient.length == 1) {
            $scope.data.clientchoisit = searchClient[0];
          }

        }
        console.log("clients", $scope.data.listtauxclients)
      }
    },
      (error) => {
        console.log(error);
    ...
    ApiListTauxOperation.getListTauxOperation().success(function (response) {
      $ionicLoading.hide();
      if (response) {
        $scope.data.listoperations = response;
        console.log('Ops', $scope.data.listoperations)
        if ($scope.details && $scope.details.idOperation) {
          var searchOp = $filter('filter')($scope.data.listoperations, { idOperation: $scope.details.idOperation })
          if (searchOp && searchOp.length == 1) {
            $scope.data.operationchoisit = searchOp[0];
          }
        }
      }
    },
    ...
    $scope.getOptOperation = function (option) {
      return option;
    };
    $scope.selectArticle();
    $scope.initvar = function () {
    ...
    $scope.submit = function () {
      ...
      if ($scope.data.clientchoisit && $scope.data.operationchoisit
        ...
        ApiAddTauxPresence.getAddTauxPresence(values, modif)
          .success(function (response) {
            $ionicLoading.hide();
            console.log(response.reponse)
            if (response.reponse == 1) {
  ...
  .factory("ApiListLocationAgent", function ($http, urlPhp) {
    return {
      getListLocationAgent: function (params) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);

        // var params = { codeUtilisateur: user.code };
        return $http.post(url + "/utilisateur/localisation.php", params);
      },
    };
  }).
...
PLANNING DESTOCKEURS*/
    return {
      getPermissionModule: function (module) {
        ...
        if (user) {
          if (user.roles && user.roles.length > 0) {
            ...
            for (var i = 0; i < user.roles.length; i++) {
              if (module == user.roles[i].libelle) {
                permission = true;
                break;
              }
            }
  ...
  .factory("ApiListTauxArticle", function ($http, urlPhp) {
    return {
      getListTauxArticle: function () {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
        // console.log(user);
        //  var params = {codeUtilisateur:user.code}
        return $http.get(url + "/tauxpresence/articles.php");
      },
    };
  })
  .factory("ApiDetailsTauxPresences", function ($http, urlPhp) {
    return {
      getDetailsTauxPresences: function (values) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
        console.log('----Detail taux presence----');
        //  var params = {codeUtilisateur:user.code}
        return $http.post(url + "/tauxpresence/details.php", values);
      },
    };
  })
  ...
  .factory("ApiUpdateTauxPresence", function ($http, urlPhp) {
    return {
      getUpdateTauxPresence: function () {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
        // console.log(user);
        //  var params = {codeUtilisateur:user.code}
        return $http.get(url + "/tauxpresence/modifier.php");
      },
    };
  })
  .factory("ApiListTauxPresences", function ($http, urlPhp) {
    return {
      getListTauxPresences: function (codeAgent) {
        ...
        // console.log(user);
        //  var params = {codeUtilisateur:user.code}
        return $http.post(url + "/tauxpresence/liste.php", codeAgent);
      },
    };
  })

  .factory("ApiListTauxOperation", function ($http, urlPhp) {
    return {
      getListTauxOperation: function () {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
        // console.log(user);
        //  var params = {codeUtilisateur:user.code}
        return $http.get(url + "/tauxpresence/listeoperation.php");
      },
    };
  })
  .factory("ApiListPdc", function ($http, urlPhp) {
    return {
      getListPdc: function (codeCommerciale) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
  ...
  .factory("ApiDetailPdc", function ($http, urlPhp) {
    return {
      getDetailPdc: function (codePDC) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
        // console.log(user);
        //  var params = {codeUtilisateur:user.code}
        var code = { codePDC: codePDC };
        return $http.post(url + "/pdc/details.php", code);
      },
    };
  })
  ...
  .factory("ApiAjoutPdc", function ($http, urlPhp) {
    return {
      ajoutPdc: function (values) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
        // console.log(user);
        //  var params = {codeUtilisateur:user.code}
        return $http.post(url + "/pdc/ajout.php", values);
      },
    };
  })
  ...
  .factory("ApiModificationDetailPDC", function ($http, urlPhp) {
    return {
      modificationDetailPDC: function (values) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);
  ...
  .factory("ApiDeletDetailPDC", function ($http, urlPhp) {
    return {
      deletDetailPDC: function (values) {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem("user");
        user = JSON.parse(user);

        return $http.post(url + "/pdc/supprimerDetails.php", values);
      },
    };
  })
  ...
  .factory('ApiListPlanningUser', function ($http, urlPhp) {
    return {
      ListPlanningUser: function () {
        var url = urlPhp.getUrl();
        var user = localStorage.getItem('user');
        console.log('-------------User-------');
        console.log(user);
        ...
        return $http.get(url + '/utilisateur/liste.php');
      }
    }
  ...
  .factory("formatNewDate", function () {
    ...
    return {
      formatNewDate: function (dateParams = null) {
        var d = new Date();
        if (dateParams) {
          d = dateParams;
        }
